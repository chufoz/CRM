...
//function prCalendar($fromyear,$frommonth,$fromday,$href,$username,$session) {
	...
	if (!$MainPageCalendar) {
		...
		print "<table width='100%'><tr><td><a class='bigsort' href='calendar.php?strmonth=" . $prevmonth . "&stryear=" . $prevyear . "&this=" . $this . "'>prev</a></td><td align='right'><a class='bigsort' href='calendar.php?strmonth=" . $nextmonth . "&stryear=" . $nextyear . "&pred=" . $nextpred . "&this=" . $this . "'>next</a></td></tr></table>";
...
//				echo "<tr align=right>\n<td class='smallsort' align='center'><b>$weeknum</b>\n";
				if (!$NoClickToWeek) {
					echo "<tr align=right>\n<td class='smallsort' align='center'><b><a href='calendar.php?weekdetail=$timestamp&year=$year$nonavins&select=$select&nav=$nav' class='bigsort' alt='Go to weekview'>$weeknum</a></b>\n";
				} else {
					echo "<tr align=right>\n<td class='smallsort' align='center'><b>$weeknum</b>\n";
				}
...
//				echo "<tr align=right>\n<td class='smallsort' align='center'><b>$weeknum</b>";
				...
				if (!$NoClickToWeek) {
					echo "<tr align=right>\n<td class='smallsort' align='center'><b><a href='calendar.php?weekdetail=$timestamp&year=$year$nonavins&select=$select' class='bigsort' alt='Go to weekview'>$weeknum</a></b>\n";
				} else {
					echo "<tr align=right>\n<td class='smallsort' align='center'><b>$weeknum</b>\n";
				}
	...
	for ($i=$GLOBALS['CAL_MINHOUR'];$i<$GLOBALS['CAL_MAXHOUR']+1;$i+=.5) {
		...
		for ($c=0;$c<$NUMDAYS;$c++) {
			...
			if (!in_array($sql,$sqla)) {
				...
				if ($p==6) {
					$ins .= "<br><a href='summary.php?owner=all&assignee=all&$GLOBALS[TBL_PREFIX]customer=all&duedate=$dd&csv=onscreenfull&extended=1' class='smallsort'>&lt;more&gt;</a>";
				}
